<template>
  <NuxtLink :to="`/news/${news.id}`" class="group block bg-stone-50 dark:bg-stone-800/60 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col border border-stone-200 dark:border-stone-700">
    ...
    <div class="p-6 flex-1 flex flex-col">
      ...
      <p class="text-stone-600 dark:text-stone-400 text-sm leading-relaxed mb-4 line-clamp-3 flex-1">
        {{ news.excerpt }}
      </p>
      <div class="mt-auto flex items-center justify-between">
        <span class="inline-flex items-center gap-1 text-orange-600 dark:text-orange-400 font-medium text-sm group-hover:gap-2 transition-all">
